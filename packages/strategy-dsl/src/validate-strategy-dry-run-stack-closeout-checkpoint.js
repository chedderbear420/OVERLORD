...
import {
  defaultStrategyDryRunStackCloseoutPaths,
  requiredStrategyDryRunCloseoutCheckNames,
  strategyDryRunStackCloseoutArtifactContracts
} from "./build-strategy-dry-run-stack-closeout-checkpoint.js";
...
const defaultFixturePath = path.join(repoRoot, "packages", "strategy-dsl", "fixtures", "synthetic_strategy_dry_run_stack_closeout_checkpoint.json");
...
const allowedStatuses = new Set(["dry_run_stack_closeout_ready", "dry_run_stack_closeout_rejected"]);
const allowedCheckStatuses = new Set(["check_passed", "check_failed", "check_not_applicable"]);
const requiredCheckNames = new Set(requiredStrategyDryRunCloseoutCheckNames);
const contractByType = new Map(strategyDryRunStackCloseoutArtifactContracts.map((contract) => [contract.artifact_type, contract]));
const expectedPaths = Object.fromEntries(strategyDryRunStackCloseoutArtifactContracts.map((contract) => [contract.artifact_type, defaultStrategyDryRunStackCloseoutPaths[contract.pathKey]]));
const forbiddenCommandPattern = /\b(curl|wget|fetch|powershell|pwsh|invoke-webrequest|invoke-restmethod|iwr|irm)\b|https?:\/\/|[|<>]/i;

export async function validateStrategyDryRunStackCloseoutCheckpointFile(options = {}) {
  ...
  return makeReport(filePath, (await validateStrategyDryRunStackCloseoutCheckpoint(checkpoint, options)).errors);
}

export async function validateStrategyDryRunStackCloseoutCheckpoint(checkpoint, options = {}) {
  const errors = [];
  const root = options.repoRoot ?? repoRoot;

  if (!checkpoint || typeof checkpoint !== "object" || Array.isArray(checkpoint)) {
    return { ok: false, errors: ["StrategyDryRunStackCloseoutCheckpoint must be a JSON object"] };
  }
...
export function formatStrategyDryRunStackCloseoutCheckpointValidationReport(report) {
  const lines = [
    "Overlord StrategyDryRunStackCloseoutCheckpoint Validation",
...
function validateCoreFields(errors, checkpoint) {
  if (checkpoint.schema_version !== "strategy_dry_run_stack_closeout_checkpoint.v1") errors.push("schema_version must be strategy_dry_run_stack_closeout_checkpoint.v1");
  ...
  if (checkpoint.status === "dry_run_stack_closeout_ready") {
    if (checkpoint.readiness_status !== "dry_run_ready") errors.push("ready closeout checkpoints require dry_run_ready readiness_status");
...
function validateIdShapes(errors, checkpoint) {
  if (typeof checkpoint.strategy_dry_run_stack_closeout_checkpoint_id !== "string" || !checkpoint.strategy_dry_run_stack_closeout_checkpoint_id.startsWith("sdrscc_")) errors.push("strategy_dry_run_stack_closeout_checkpoint_id must reference a StrategyDryRunStackCloseoutCheckpoint id");
  ...
  if (typeof checkpoint.source_strategy_run_manifest_id !== "string" || !checkpoint.source_strategy_run_manifest_id.startsWith("srm_")) errors.push("source_strategy_run_manifest_id must reference a StrategyRunManifest id");
  if (typeof checkpoint.source_strategy_run_evidence_bundle_id !== "string" || !checkpoint.source_strategy_run_evidence_bundle_id.startsWith("sreb_")) errors.push("source_strategy_run_evidence_bundle_id must reference a StrategyRunEvidenceBundle id");
  if (typeof checkpoint.source_strategy_dry_run_plan_id !== "string" || !checkpoint.source_strategy_dry_run_plan_id.startsWith("sdrp_")) errors.push("source_strategy_dry_run_plan_id must reference a StrategyDryRunPlan id");
  if (typeof checkpoint.source_strategy_dry_run_plan_evidence_summary_id !== "string" || !checkpoint.source_strategy_dry_run_plan_evidence_summary_id.startsWith("sdrpes_")) errors.push("source_strategy_dry_run_plan_evidence_summary_id must reference a StrategyDryRunPlanEvidenceSummary id");
  if (typeof checkpoint.source_strategy_dry_run_readiness_checkpoint_id !== "string" || !checkpoint.source_strategy_dry_run_readiness_checkpoint_id.startsWith("sdrpc_")) errors.push("source_strategy_dry_run_readiness_checkpoint_id must reference a StrategyDryRunReadinessCheckpoint id");
  if (typeof checkpoint.source_strategy_dry_run_noop_summary_id !== "string" || !checkpoint.source_strategy_dry_run_noop_summary_id.startsWith("sdrns_")) errors.push("source_strategy_dry_run_noop_summary_id must reference a StrategyDryRunNoOpSummary id");
  if (typeof checkpoint.source_strategy_dry_run_evidence_bundle_id !== "string" || !checkpoint.source_strategy_dry_run_evidence_bundle_id.startsWith("sdreb_")) errors.push("source_strategy_dry_run_evidence_bundle_id must reference a StrategyDryRunEvidenceBundle id");
  if (typeof checkpoint.source_strategy_dry_run_case_file_summary_id !== "string" || !checkpoint.source_strategy_dry_run_case_file_summary_id.startsWith("sdrcfs_")) errors.push("source_strategy_dry_run_case_file_summary_id must reference a StrategyDryRunCaseFileSummary id");
}

function validateDeterministicId(errors, checkpoint) {
  const expected = strategyDryRunStackCloseoutCheckpointId({
    ...
    strategyRunManifestId: checkpoint.source_strategy_run_manifest_id,
    strategyRunEvidenceBundleId: checkpoint.source_strategy_run_evidence_bundle_id,
    strategyDryRunPlanId: checkpoint.source_strategy_dry_run_plan_id,
    strategyDryRunPlanEvidenceSummaryId: checkpoint.source_strategy_dry_run_plan_evidence_summary_id,
    strategyDryRunReadinessCheckpointId: checkpoint.source_strategy_dry_run_readiness_checkpoint_id,
    strategyDryRunNoopSummaryId: checkpoint.source_strategy_dry_run_noop_summary_id,
    strategyDryRunEvidenceBundleId: checkpoint.source_strategy_dry_run_evidence_bundle_id,
    strategyDryRunCaseFileSummaryId: checkpoint.source_strategy_dry_run_case_file_summary_id,
    freezeRecommendation: checkpoint.freeze_recommendation
  });
  if (checkpoint.strategy_dry_run_stack_closeout_checkpoint_id !== expected) {
    errors.push("strategy_dry_run_stack_closeout_checkpoint_id must be deterministic from closeout source ids and freeze recommendation");
  }
}
...
function validateSourceArtifactAlignment(errors, checkpoint) {
  ...
  const expected = {
    strategy_definition: checkpoint.source_strategy_definition_id,
    strategy_run_intent: checkpoint.source_strategy_run_intent_id,
    strategy_run_manifest: checkpoint.source_strategy_run_manifest_id,
    strategy_run_evidence_bundle: checkpoint.source_strategy_run_evidence_bundle_id,
    strategy_dry_run_plan: checkpoint.source_strategy_dry_run_plan_id,
    strategy_dry_run_plan_evidence_summary: checkpoint.source_strategy_dry_run_plan_evidence_summary_id,
    strategy_dry_run_readiness_checkpoint: checkpoint.source_strategy_dry_run_readiness_checkpoint_id,
    strategy_dry_run_noop_summary: checkpoint.source_strategy_dry_run_noop_summary_id,
    strategy_dry_run_evidence_bundle: checkpoint.source_strategy_dry_run_evidence_bundle_id,
    strategy_dry_run_case_file_summary: checkpoint.source_strategy_dry_run_case_file_summary_id
  };
  ...
  const traceArtifact = artifactByType.get("strategy_dry_run_trace");
  if (traceArtifact && traceArtifact.artifact_id !== null) errors.push("strategy_dry_run_trace artifact_id must be null");
}
...
async function validateCloseoutArtifacts(errors, root, artifacts) {
  ...
  for (const artifact of artifacts) {
    ...
    if (!contract) {
      ...
      if (artifact.artifact_path !== expectedPaths[artifact.artifact_type]) errors.push("closeout artifact_path must match the known dry-run closeout contract");
      if (artifact.schema_version !== contract.schema_version) errors.push("closeout artifact schema_version must match the known dry-run closeout contract");
      if (artifact.validation_command !== contract.validation_command) errors.push("closeout artifact validation_command must match the known dry-run closeout contract");
    }
  ...
  for (const contract of strategyDryRunStackCloseoutArtifactContracts) {
...
function validateCloseoutChecks(errors, checks, status, freezeRecommendation) {
  ...
  if (status === "dry_run_stack_closeout_ready" && checks.some((check) => requiredCheckNames.has(check.check_name) && check.status !== "check_passed")) {
    errors.push("ready closeout checkpoints require all required closeout checks to pass");
  }
  if (hasFailedRequiredCheck) {
    if (status !== "dry_run_stack_closeout_rejected") errors.push("failed closeout checks require dry_run_stack_closeout_rejected status");
    if (freezeRecommendation !== "freeze_not_ready") errors.push("failed closeout checks require freeze_not_ready");
  }
}
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  console.log(formatStrategyDryRunStackCloseoutCheckpointValidationReport(report));

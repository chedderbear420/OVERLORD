...
**ACTIVE** (branch: `phase-4p-paper-ledger-entry`)

## Purpose

Phase 4P introduces the `KalshiPaperLedgerEntry` artifact. It records the hypothetical economics of a Phase 4N signal observation — one contract unit, unsettled — using the Phase 4O `KalshiSignalEvaluationSummary` and Phase 4M `KalshiMarketSnapshot` as inputs.
...
├── paper_realized_pnl_cents             null (unsettled in Phase 4P)
├── paper_fees_cents                     0 (default)
├── paper_net_pnl_cents                  unrealized + fees
├── paper_contract_quantity              1
├── paper_settlement_status              "unsettled"
├── ledger_entry_status                  "recorded_non_actionable"
├── data_quality_status                  "complete"
├── research_summary
│   ├── entry_recorded                   true (always)
│   ├── settlement_complete              false (unsettled)
│   └── pnl_realized                     false (null realized PnL)
...
├── reason_code                          "LEDGER_ENTRY_RECORDED_NON_ACTIONABLE"
└── reason                               "paper ledger entry recorded non-actionably"
```
...
- `paper_realized_pnl_cents` — always `null` in Phase 4P (unsettled).
- `paper_fees_cents` — always `0` in Phase 4P (no fee model yet).
- `paper_net_pnl_cents` — `unrealized + fees` when unsettled.
...
| `packages/strategy-dsl/tests/build-kalshi-paper-ledger-entry.test.js` | 11 builder tests |
| `packages/strategy-dsl/tests/kalshi-paper-ledger-entry-validation.test.js` | 33 validation tests |
...
- `paper_realized_pnl_cents` is always `null` (unsettled); validator enforces this
- `paper_net_pnl_cents` is validated for consistency with `paper_unrealized_pnl_cents + paper_fees_cents`
